...
#gui_start_of_page( "<fmt:message key="imcms/lang/jsp/document_list.jsp/title"/>" "AdminManager" "" "ListDocument" "" )
...
<tr>
	...
	<tr>
		...
		<td>&nbsp;&nbsp;</td>
		<td><input type="submit" class="imcmsFormBtnSmall" name="<%= ListDocuments.PARAMETER_BUTTON__LIST %>" value=" <fmt:message key="imcms/lang/jsp/document_list.jsp/2002"/> "></td>
	</tr>
	</form>
	</table></td>
</tr>
...
<tr valign="top"><%String alias = document.getAlias();
	...
	<a href="<%= request.getContextPath() %>/servlet/DocumentReferences?<%= DocumentReferences.REQUEST_PARAMETER__RETURNURL %>=<%= escapedBackUrl %>&<%= DocumentReferences.REQUEST_PARAMETER__REFERENCED_DOCUMENT_ID %>=<%= document.getId() %>"><%
		} %><%= documentMenuPairsContainingDocument.length %> <fmt:message key="web/imcms/lang/jsp/parent_count_unit"/><%
		if (documentMenuPairsContainingDocument.length > 0 ) {
			%></a><%
		} %></td>
	<td><%
...
<tr>
	<td colspan="6" align="right"><input type="submit" class="imcmsFormBtn" name="" value="<fmt:message key="global/cancel"/>"></td>
</tr>
</form>
</table><%
} %>
