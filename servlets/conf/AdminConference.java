	...
	private void listConferences( HttpServletRequest request, HttpServletResponse response, String languagePrefix )
		...
		*/
		...
		if ( noErrors ) {
			...
			String[][] listOfConferences = IMCServiceRMI.sqlQueryMulti( imcserver, "ListConferences" );
			...
			for ( int i = 0 ; i < listOfConferences.length ; i++ ) {
				...
				if ( queryResultForum.length > 0 ) {
					conferenceTags.put( "SERVLET_URL", MetaInfo.getServletPath( request ) );
